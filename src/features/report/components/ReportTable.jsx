      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="h-20 border-b border-neutral-200 bg-white text-left">
            <th className="w-20 px-4 text-sm font-medium leading-5 text-neutral-600 whitespace-nowrap">
              신고 ID
            </th>
            <th className="px-4 text-sm font-medium leading-5 text-neutral-600 whitespace-nowrap">
              콘텐츠 유형
            </th>
        ...
        <tbody>
          {reports.length === 0 ? (
            ...
            reports.map((report) => (
              <tr
                key={`${report.contentType}-${report.reportId}`}
                className="border-b border-neutral-200"
              >
                <td className="px-4 py-6 text-sm text-neutral-950">{report.reportId}</td>
                <td className="px-4 py-6 text-sm text-neutral-950 whitespace-nowrap">
                ...
                <td className="px-6 py-6 align-middle">
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        PATH.REPORT_DETAIL.replace(':contentType', report.contentType).replace(
                          ':reportId',
                          report.contentId
                        )
                      )
